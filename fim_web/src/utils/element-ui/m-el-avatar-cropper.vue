...
import {ApiFile} from "@/api/api_file.ts";
...
/* 父组件传参 */
const props = defineProps<m_type_el.props.AvatarCropper>()
// 裁剪组件需要使用到的参数
...
const acceptType = ref<string[]>([])  // 允许上传的类型
const previews: any = ref({})         // 裁剪后的预览样式信息
const previewFileStyle = ref({})
const cropperRef: any = ref({})       // 裁剪组件Ref
const reuploadInput = ref<HTMLElement | null | undefined>() // input组件Ref
// 回显图片使用的方法
...
const onConfirm = () => {
  cropperRef.value.getCropData(async (data: string) => {
    ...
    // 上传成功
    const response = await ApiFile.Upload(dataFile, "avatar")
    if (!baseResponse(response)) return;

    // 触发自定义事件
    emits('confirm', response.data.url)
